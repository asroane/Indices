...
#Erini Lambrides' code adapted from https://github.com/elambrid/bdnyc/blob/master/avgflux.py to work with astrodbkit.
...
def avgflux(startw, endw, dfspec,i):
	...
	if max(dfspec['wavelength'][i]) > startw and min(dfspec['wavelength'][i]) < endw: 
		...
		wavelength = dfspec['wavelength'][i][(dfspec['wavelength'][i]+pix_scale/2 >= startw) & (dfspec['wavelength'][i]-pix_scale/2 <= endw)] #figure out how to call parts of the pandas dataframe?
		#print wavelength
